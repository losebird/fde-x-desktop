...
export function SpecFeed({ app, view, workspaceCwd, previewRows, reloadToken }: Props) {
  ...
  return (
    <div className="border border-line rounded-lg overflow-hidden" data-app-feed="true">
      ...
      {error && <div className="px-3 py-2 text-xs text-accent-red">{error}</div>}
      {rows.length === 0 ? (
        ...
        <div className="divide-y divide-line">
          {rows.map((row) => (
            <div key={String(row.id)} className="px-3 py-2.5 flex items-start gap-3" data-app-feed-row="true">
              <div className="min-w-0 flex-1">
                <div className="text-sm truncate">{String(row[titleField] ?? row.id)}</div>
                <div className="text-[11px] text-ink-muted mt-0.5 flex flex-wrap gap-2">
                  {enumField && row[enumField.name] != null && (
                    <span className="border border-line rounded px-1">{String(row[enumField.name])}</span>
                  )}
